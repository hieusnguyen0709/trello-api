...
describe('API Integration: PUT /v1/cards/:id/restore', () => {
    let testUser
    let accessToken
    let testBoard
    let testColumn
    ...
    it('Should return 200 OK and restore the card', async () => {
        const res = await request
            .put(`/v1/cards/${testCard._id}/restore`)
    ...
    it('Should set updatedAt when restoring the card', async () => {
        const res = await request
            .put(`/v1/cards/${testCard._id}/restore`)
            .set('Cookie', [`accessToken=${accessToken}`])
    ...
    it('Should not delete the card from database when restored', async () => {
        const res = await request
            .put(`/v1/cards/${testCard._id}/restore`)
            .set('Cookie', [`accessToken=${accessToken}`])
    ...
    it('Should return 404 when the card does not exist', async () => {
        ...
        const res = await request
            .put(`/v1/cards/${nonExistentCardId}/restore`)
            .set('Cookie', [`accessToken=${accessToken}`])
    ...
    it('Should return 422 when card id is invalid', async () => {
        const res = await request
            .put('/v1/cards/invalid-card-id/restore')
            .set('Cookie', [`accessToken=${accessToken}`])
